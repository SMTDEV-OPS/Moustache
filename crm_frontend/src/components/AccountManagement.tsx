...
import { Loader2, Plus, Trash2, Edit, Search, Building2, MapPin, Phone, Mail, User, ChevronRight, ChevronDown, Network, GitBranch, UserPlus, Archive, Upload } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function FollowUpDateCell({
  ...
  const [followUpDate, setFollowUpDate] = useState<Date | null | undefined>(
  ...
  }, [open, account.followUpDate, account.followUpNote]);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <span className="text-xs text-primary">{account.followUpDate ? "Set Follow Up" : "Set Follow Up"}</span>
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-80" align="start" onClick={(e) => e.stopPropagation()}>
        <div className="space-y-4">
          <div>
            <Label className="text-xs font-bold text-muted-foreground uppercase">Follow-up Date</Label>
            <DatePicker
              value={followUpDate}
...
export const AccountManagement = ({ permissions = [], isAdmin, isSystemAdmin }: AccountManagementProps = {}) => {
  ...
  if (isLoading && accounts.length === 0) {
    return (
      <PageShell title="Account Management" description="Manage travel agents, corporates, and event planners">
    ...
    return (
      <>
        ...
        <AccountCreationWizard
          ...
          canAssignManagers={!!isAdmin || permissions?.includes("accounts.assign_managers") || permissions?.includes("accounts.manage")}
          onSuccess={async () => {
            const list = await loadData();
      ...
      actions={
        <div className="flex items-center gap-3">
          ...
          {canUpdateAccounts && (
            <div className="flex items-center gap-2 border border-border rounded-md p-1">
              <Button
                variant={accountScope === "my" ? "default" : "ghost"}
                size="sm"
              ...
              <Button
                variant={accountScope === "all" ? "default" : "ghost"}
                size="sm"
          ...
          {canCreateAccounts && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => { setImportDialogOpen(true); setImportResult(null); setImportFile(null); }}
            >
          ...
          {canCreateAccounts && (
            <Button
              ...
              variant="default"
              className="rounded-none px-8 py-6 font-medium tracking-wide"
            >
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input
                ...
                className="pl-10 h-10 rounded-md border border-slate-200/90 bg-white shadow-none focus-visible:border-slate-300 focus-visible:ring-2 focus-visible:ring-slate-200/50 dark:border-slate-600/50 dark:bg-slate-900/30 dark:focus-visible:border-slate-500"
                value={searchQuery}
      ...
      {viewMode === "hierarchy" ? (
          ...
          <CardContent>
            {rootAccounts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-base font-medium text-foreground mb-1">No accounts found</p>
                <p className="text-sm text-muted-foreground mb-4">Create your first account to get started</p>
                <Button onClick={() => handleOpenWizard()} className=" rounded-none">
                  <Plus className="mr-2 h-4 w-4" />
        ...
        <Card className="border-border shadow-sm">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-base font-medium text-foreground mb-1">
              {accounts.length === 0 ? "No accounts created yet" : searchQuery ? `No account found for "${searchQuery}"` : "No accounts match your filters"}
            </p>
            <p className="text-sm text-muted-foreground mb-6 text-center max-w-xs">
              {accounts.length === 0 ? "Create your first account" : searchQuery ? "Try a different search" : "Try adjusting your filters"}
            </p>
            {(accounts.length === 0 || searchQuery) && (
              <Button onClick={() => handleOpenWizard()} variant="default"><Plus className="mr-2 h-4 w-4" />Create New Account</Button>
            )}
          </CardContent>
        </Card>
      ) : (
        <DataTable<Account>
          ...
          columns={[
            ...
            { id: "children", header: "Children", render: (a) => <span className="text-sm text-muted-foreground">{accounts.filter(x => x.parentAccountId === a.id).length}</span> },
          ]}
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload an .xlsx file. Required column: "Company Name".
