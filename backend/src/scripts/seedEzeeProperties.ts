...
export async function seedEzeeProperties(): Promise<{ upserted: number; created: number }> {
  let upserted = 0;
  let created = 0;
  ...
  logger.info(`seedEzeeProperties: ${PROPERTIES.length} targets, ${upserted} upserted, ${created} created`);
